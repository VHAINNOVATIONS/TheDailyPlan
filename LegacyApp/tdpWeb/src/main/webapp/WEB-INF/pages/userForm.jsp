...
<s:form name="userForm" action="saveUser" method="post" validate="true">
    ...
    <s:label key="user.username" cssClass="text"/>
    ...
    <li>
        <div>
            <s:textfield key="user.email" theme="xhtml" cssClass="text medium"/>
        </div>
    </li>

    <li>
        <label class="desc"><fmt:message key="user.prefs.prefs"/></label>
        <div class="group">
        	<ul>
               	<s:radio key="user.prefInitialSearch" list="tabList"></s:radio>
               	<s:select key="user.prefFontSize" list="fontList" listKey="value" listValue="display" />
                <s:select key="user.prefTeam" list="vistaTeams" 
	                headerKey="-1" headerValue="Select Team"
