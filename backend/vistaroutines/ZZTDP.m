...
POSTING(IEN) ;
 ...
 ;
PSTNTYPE()  ;
 N TIUPPCW,TIUPPCN,TIUPPADV,TIUPPADD,ID,NDX,CID,NAME,RESULT
 D DOCDEF^TIUPP3
 F ID=TIUPPCW,TIUPPCN,TIUPPADV,TIUPPADD D
 . Q:ID<1
 . S NDX=0
 . F  S NDX=$O(^TIU(8925.1,ID,10,NDX)) Q:'NDX  D
 . . S CID=$P(^TIU(8925.1,ID,10,NDX,0),"^",1)
 . . S NAME=$P(^TIU(8925.1,CID,0),"^",1)
 . . I $D(RESULT) S RESULT=RESULT_"^"_NAME
 . . E  S RESULT=NAME
 Q RESULT
 ;
BOILPLTS(DFN,TEXT) ;
