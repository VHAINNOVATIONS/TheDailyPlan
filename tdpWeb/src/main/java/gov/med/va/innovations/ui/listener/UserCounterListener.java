...
public class UserCounterListener implements ServletContextListener, HttpSessionAttributeListener {
    ...
    synchronized void removeMdwsClient(HttpSession session) {
    	ApplicationContext ctx = 
    	      WebApplicationContextUtils.
    	         getWebApplicationContext(session.getServletContext());
    	 
        MdwsClientFactory clients =
    	     (MdwsClientFactory) ctx.getBean("mdwsClientFactory");
    	clients.remove();
    }
    ...
    public void attributeRemoved(HttpSessionBindingEvent event) {
        if (event.getName().equals(EVENT_KEY) && !isAnonymous()) {
            ...
            if (auth != null && (auth.getPrincipal() instanceof User)) {
                ...
                removeUsername(user);
                removeMdwsClient(event.getSession());
            }
        }
    }
