...
public class StartupListener implements ServletContextListener {
    ...
    public void contextInitialized(ServletContextEvent event) {
        ...
        try {
            ...
            for (Object o : provider.getProviders()) {
                ...
                if (p instanceof RememberMeAuthenticationProvider && !"SpringSecured".equals(((RememberMeAuthenticationProvider)p).getKey())) {
    ...
    /**
     * This method will reload all common definitions from MedlinePlus
     */
    private static void initializeData(ServletContext context) {
        ApplicationContext ctx = WebApplicationContextUtils.getRequiredWebApplicationContext(context);
        try {
	        DefinitionManager mgr = (DefinitionManager) ctx.getBean("definitionManager");
	        
//	        mgr.reloadCommonMedlineDefs();
	    } catch (NoSuchBeanDefinitionException n) {
	        log.debug("definitionManager bean not found, assuming test and ignoring...");
	        // ignore, should only happen when testing
	    }
	}
    ...
    public static void setupContext(ServletContext context) {
        ...
        context.setAttribute(Constants.AVAILABLE_COMPONENTS, mgr.getAllComponents());
