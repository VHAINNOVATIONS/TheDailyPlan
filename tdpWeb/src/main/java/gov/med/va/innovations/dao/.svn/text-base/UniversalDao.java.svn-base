 ...
 * 
 * @see gov.med.va.innovations.dao.GenericDao
 */
public interface UniversalDao {
    ...
    /**
     * Used for long paging lists, so that the entire result set doesn't need to be retrieved
     * 
     * @param clazz model class to lookup
     * @param criteria - placed into a where clause
     * @param firstResult - beginning row
     * @param maxResults - number of rows to retrieve, which is generally the page size
     * @return - list of results
     */
    List findByCriteria(Class clazz, String criteria, int firstResult, int maxResults);
    
    /**
     * Count the number of rows in the table represented by the model class
     * @param clazz
     * @return
     */
